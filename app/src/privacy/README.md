...
The alpha.1 production build did every native PIN derivation in Hermes JavaScript, even though the work factor was designed for password hashing. The worker moves that expensive calculation away from rendering. This is a confirmed code-path bottleneck, not a claim that the reported phone hang has been reproduced on that device.
