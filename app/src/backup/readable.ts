...
import { installmentAllocations, isDebt, reductionAmounts } from '../ledger';
...
function tables(data: BackupPayload): Table[] {
  ...
  const amounts = new Map(data.tx.filter(isDebt).map(t => [t.id, reductionAmounts(data.tx, t)]));
  const remaining = (t: typeof data.tx[number]) => toCents(amounts.get(t.id)?.remainingAmount ?? 0);
  const kind = (t: typeof data.tx[number]) => t.dir === 'me' ? 'دين لي' : t.dir === 'owe' ? 'دين عليّ' :
    t.dir === 'forgive' ? txById.get(t.debtId!)?.dir === 'me' ? 'إعفاء للشخص' : 'إعفاء من الشخص' :
    txById.get(t.debtId!)?.dir === 'me' ? 'سداد مستلم' : 'سداد مدفوع';
  const status = (t: typeof data.tx[number]) => {
    if (t.voidedAt) return 'ملغاة';
    const a = amounts.get(t.id);
    if (!a) return 'سارية';
    if (a.remainingAmount > 0) return a.forgivenAmount > 0 ? 'إعفاء جزئي' : a.paidAmount > 0 ? 'مسدد جزئياً' : 'مفتوح';
    return a.forgivenAmount === t.amount ? 'معفى بالكامل' : a.forgivenAmount > 0 ? 'مغلق بسداد وإعفاء' : 'مسدد بالكامل';
  };
  ...
  return [
    { name: 'iou-balances.csv', title: 'أرصدة الأشخاص',
      ...
      rows: data.people.map(p => {
        const debts = data.tx.filter(t => t.personId === p.id && !t.voidedAt && isDebt(t));
    ...
    { name: 'iou-transactions.csv', title: 'الديون والدفعات والإعفاءات',
      displayOrder: [2, 3, 4, 13, 14, 12, 5, 8, 10, 7, 6, 11, 9, 0, 1],
      headers: ['معرّف العملية', 'معرّف الشخص', 'الشخص', 'النوع', 'المبلغ (ر.س)', 'تاريخ العملية', 'وقت التسجيل', 'تاريخ الاستحقاق', 'الملاحظة', 'معرّف الدين المرتبط', 'الحالة', 'وقت الإلغاء', 'المتبقي (ر.س)', 'المسدد (ر.س)', 'المعفى منه (ر.س)'],
      rows: data.tx.map(t => [t.id, t.personId, names.get(t.personId) ?? '', kind(t), t.amount, t.createdAt,
        t.recordedAt ?? t.createdAt, t.dueAt ?? '', t.note ?? '', t.debtId ?? '', status(t), t.voidedAt ?? '', money(remaining(t)),
        amounts.get(t.id)?.paidAmount ?? (t.dir === 'settle' && !t.voidedAt ? t.amount : 0),
        amounts.get(t.id)?.forgivenAmount ?? (t.dir === 'forgive' && !t.voidedAt ? t.amount : 0)]) },
    { name: 'iou-installments.csv', title: 'الأقساط',
      displayOrder: [1, 2, 3, 7, 8, 5, 4, 6, 0],
      headers: ['معرّف الدين', 'الشخص', 'القسط', 'المبلغ (ر.س)', 'تاريخ الاستحقاق', 'المتبقي من القسط (ر.س)', 'حالة الدين', 'المسدد من القسط (ر.س)', 'المعفى منه من القسط (ر.س)'],
      rows: data.tx.flatMap(t => {
        const allocations = installmentAllocations(data.tx, t);
        return (t.installments ?? []).map((i, index) => {
          const a = allocations[index];
          return [t.id, names.get(t.personId) ?? '', i.label, i.amount, i.dueAt, t.voidedAt ? 0 : a.remainingAmount, status(t), a.paidAmount, a.forgivenAmount];
        });
      }) },
...
<p>تاريخ العملية هو التاريخ الفعلي؛ وقت التسجيل هو وقت إضافتها. التواريخ بالتقويم الميلادي، والأوقات المنتهية بحرف Z بتوقيت UTC. تُوزّع الدفعات والإعفاءات على الأقساط بالترتيب حسب تاريخ العملية. الإعفاء تنازل عن الدين، ويظهر مستقلاً عن الأموال المستلمة والمدفوعة. في جدول العمليات، تعرض صفوف الديون إجماليات الدفعات والإعفاءات المرتبطة بها؛ لا تجمع هذه الإجماليات مرة أخرى مع صفوف الدفعات والإعفاءات.</p>
