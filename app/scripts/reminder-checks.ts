...
import { isCurrentDelayedReminder, pendingReminderTime } from '../src/pendingReminders';
...
const observedClock = readReminderClock(new Date('2026-09-13T12:00:00Z'), 1000);
eq('clock observer records actual wall time', observedClock.wall, Date.parse('2026-09-13T12:00:00Z'));
eq('ordinary minute tick does not reschedule', reminderClockChanged(observedClock, { ...observedClock, wall: observedClock.wall + 60_000, elapsed: observedClock.elapsed + 60_000 }), false);
eq('a late timer without clock adjustment does not reschedule', reminderClockChanged(observedClock, { ...observedClock, wall: observedClock.wall + 600_000, elapsed: observedClock.elapsed + 600_000 }), false);
eq('local date rollover rebuilds schedule', reminderClockChanged(observedClock, { ...observedClock, day: '2026-09-14' }), true);
eq('timezone offset change rebuilds schedule', reminderClockChanged(observedClock, { ...observedClock, offset: observedClock.offset - 60 }), true);
eq('meaningful forward clock adjustment rebuilds schedule', reminderClockChanged(observedClock, { ...observedClock, wall: observedClock.wall + 180_000, elapsed: observedClock.elapsed + 60_000 }), true);
eq('meaningful backward clock adjustment rebuilds schedule', reminderClockChanged(observedClock, { ...observedClock, wall: observedClock.wall - 120_000, elapsed: observedClock.elapsed + 60_000 }), true);
...
const delayedWhen = new Date(2027, 0, 10, 20, 0, 0, 0).getTime();
const delayedPlan = planReminders([debt()], {}, delayedWhen - 1, defaults)[0];
const pending = { identifier: `iou-${delayedPlan.id}`, content: { ...reminderContent(delayedPlan, false), data: { debtId: 'd1' } }, trigger: { type: 'date', value: delayedWhen } };
eq('native Android DATE value read as epoch milliseconds', pendingReminderTime(pending), delayedWhen);
eq('recent due but undelivered reminder eligible for catch-up', isCurrentDelayedReminder(pending, [debt()], {}, defaults, delayedWhen + 60_000), true);
eq('pending requests older than24h are not replayed', isCurrentDelayedReminder(pending, [debt()], {}, defaults, delayedWhen + 24 * 3600000 + 1), false);
eq('future pending request is rebuilt normally', isCurrentDelayedReminder(pending, [debt()], {}, defaults, delayedWhen - 1), false);
eq('delayed reminder with edited person name removed', isCurrentDelayedReminder(pending, [debt({ personName: 'اسم آخر' })], {}, defaults, delayedWhen + 1), false);
eq('delayed reminder with edited note removed', isCurrentDelayedReminder(pending, [debt({ note: 'تغيير' })], {}, defaults, delayedWhen + 1), false);
eq('delayed reminder with changed due date removed', isCurrentDelayedReminder(pending, [debt({ nextDueAt: '2027-01-11' })], {}, defaults, delayedWhen + 1), false);
eq('voided delayed reminder removed', isCurrentDelayedReminder(pending, [debt({ voidedAt: '2027-01-10T12:00:00Z' })], {}, defaults, delayedWhen + 1), false);
eq('missing delayed debt removed', isCurrentDelayedReminder(pending, [], {}, defaults, delayedWhen + 1), false);
eq('unknown notification identifiers cannot be retained', isCurrentDelayedReminder({ ...pending, identifier: 'foreign' }, [debt()], {}, defaults, delayedWhen + 1), false);
const snoozing = { ...defaults, snoozedUntil: { d1: new Date(delayedWhen + 86400000).toISOString() } };
eq('new snooze cancels a delayed original due alert', isCurrentDelayedReminder(pending, [debt()], {}, snoozing, delayedWhen + 1), false);
const iosPending = { ...pending, trigger: { type: 'timeInterval', seconds: 60, repeats: false }, content: { ...pending.content, data: { debtId: 'd1', plannedAt: new Date(delayedWhen).toISOString() } } };
eq('iOS pending instant comes from original planned time', pendingReminderTime(iosPending), delayedWhen);
eq('iOS delayed request revalidated against current ledger', isCurrentDelayedReminder(iosPending, [debt()], {}, defaults, delayedWhen + 1), true);
eq('unknown native trigger without an original instant rejected', pendingReminderTime({ ...pending, trigger: { type: 'calendar' } }), null);
eq('malformed stored original instant rejected', pendingReminderTime({ ...iosPending, content: { ...iosPending.content, data: { plannedAt: 'invalid' } } }), null);
