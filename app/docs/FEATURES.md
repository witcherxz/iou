...
The device keeps ten dated snapshots before ledger changes and a previous copy. Startup tries each recovery source independently and displays a notice when recovering. Unreadable history is preserved before repair; failed writes remain visible and block further normal editing. Restoring a local snapshot preserves this device's backup destination and privacy lock and requires a manual first external save before automatic backup resumes. Failed folder/provider writes also pause automatic backup across restart; read-only history and cancelled restores do not clear that pause.

Folder backups keep ten verified immutable JSON snapshots, compatibility mirrors, a standalone HTML report, and four CSV companions. Default recovery chooses the newest valid copy by its backup timestamp, even when the main file is valid but older. Each native shared export uses a separate cached file, so a later export cannot replace bytes a receiving app is still reading. The manual HTML report opens offline without IoU and contains downloadable spreadsheet tables and the full recoverable JSON. Imports validate the data without executing imported HTML. See [backup formats and encryption](BACKUPS.md).
...
Reminders stop for closed (paid or forgiven), cancelled, or disabled debts, offer advance notices and overdue follow-ups, and keep at most 60 upcoming debt notifications plus the weekly reminder. The schedule refreshes on app foregrounding and while open after day, timezone or significant clock changes. Blocked Android notifications or channels offer a route to system settings. Recent still-pending reminders can catch up only while their current debt, preferences and content still match. Browser preview saves preferences and explains that native delivery is unavailable there. See [scheduling details](FLEXIBLE_REMINDERS.md).
