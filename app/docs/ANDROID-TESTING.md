...
## Published Beta 1 — final APK acceptance

The independently verified release APK installed over the existing synthetic emulator ledger. Existing PIN unlock, Home/person/debt navigation, record counts and balance breakdown all passed. The full **إعفاء جزئي** badge is visibly unclipped. A new Unicode-password protected export from this exact APK independently decrypted in Node and opened as a local file in offline Chrome: wrong passwords are rejected, every portable field matches the baseline apart from the expected export timestamp, and all four tables/CSV downloads agree exactly. No HTTP request or browser error occurred; earlier shared files remained byte-identical. No financial, preference or folder writes were performed, and clipboard/helper cleanup completed. [Final native and browser evidence](verification/beta1-native.json).

| Home | Person | Full status and balance breakdown |
|---|---|---|
| ![Preserved synthetic ledger](screenshots/beta1-home.png) | ![Preserved person history](screenshots/beta1-person.png) | ![Complete partial forgiveness badge](screenshots/beta1-debt.png) |
