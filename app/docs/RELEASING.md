...
The application version is `0.1.0-alpha.9`, Android version code `9`, and production application ID `io.github.witcherxz.iou`. The same version is recorded in `app.json`, `package.json`, and the package-lock root. iOS uses the numeric marketing version `0.1.0` and build number `9` if built separately.
...
4. Push a matching version tag, such as `v0.1.0-alpha.9`. Actions builds and verifies the APK, then publishes it and its `.apk.sha256` checksum to a GitHub prerelease. Tag/version mismatches fail.
...
```bash
sha256sum -c iou-0.1.0-alpha.9.apk.sha256
```
